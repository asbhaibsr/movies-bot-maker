# Don't Remove Credit AsBhai BSR
# Subscribe YouTube Channel For Amazing Bot AsBhai BSR
# Ask Doubt on telegram AsBhai BSR
...
from CloneAsBhai.database.clone_bot_userdb import clonedb
from info import *
...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if f_caption is None:
                f_caption = f"AsBhai BSR {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1['file_name'].split()))}"
            if cd["update_channel_link"] != None:
                up = cd["update_channel_link"]
                button = [[
    ...
    if f_caption is None:
        f_caption = f"AsBhai BSR {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files['file_name'].split()))}"
    if cd["update_channel_link"] != None:
        up = cd["update_channel_link"]
        button = [[
            InlineKeyboardButton('🍿 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ 🍿', url=up)
