...
from AsBhai.util.human_readable import humanbytes
from AsBhai.util.file_properties import get_file_ids
from AsBhai.server.exceptions import InvalidHash
import urllib.parse
import logging
import aiohttp
...
async def render_page(id, secure_hash, src=None):
    file = await AsBhaiBot.get_messages(int(LOG_CHANNEL), int(id))
    file_data = await get_file_ids(AsBhaiBot, int(LOG_CHANNEL), int(id))
